...
Dependencies that have local test stand-ins (e.g., sqlite3 in-memory for Postgres, in-memory filesystem). Deepenable if the test substitute exists. The deepened module is tested with the local stand-in running in the test suite.
